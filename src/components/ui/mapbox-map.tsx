...
const MapboxMap: React.FC<MapboxMapProps> = ({
  ...
  const [popupInfo, setPopupInfo] = useState<PropertyListing | null>(null);
  ...
  const fetchNearbyListings = useCallback(async (centerLat: number, centerLng: number, currentZoom: number = zoom) => {
    ...
    try {
      const radius = currentZoom >= 16 ? 0.02 : currentZoom >= 14 ? 0.05 : 0.1;
      ...
      <div className={`relative ${className}`} style={containerStyle}>
        <div className="absolute inset-0 bg-muted flex items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground">Mapbox access token is not configured</p>
          </div>
        </div>
      </div>
    );
        ...
        >
          ...
          {showMarker && latitude && longitude && (
            <Marker
              ...
              anchor="bottom"
            >
              <div className="bg-green-600 text-white px-3 py-1.5 rounded-lg shadow-lg font-medium text-xs cursor-pointer hover:bg-green-700 transition-colors whitespace-nowrap">
          ...
          {nearbyListings.map((listing) => {
            ...
            return (
              ...
              >
                <div
                  className={`bg-white text-gray-900 px-3 py-1.5 rounded-lg shadow-md font-semibold text-xs cursor-pointer hover:shadow-lg transition-all whitespace-nowrap ${
          ...
          {popupInfo && popupInfo.map.latitude && popupInfo.map.longitude && (
            <Popup
              longitude={popupInfo.map.longitude}
              latitude={popupInfo.map.latitude}
              anchor="bottom"
              onClose={() => setPopupInfo(null)}
              closeButton={true}
            >
              <div className="p-2">
                <p className="font-semibold">{popupInfo.details.propertyType}</p>
                <p className="text-sm">{formatPrice(popupInfo.listPrice)}</p>
              </div>
            </Popup>
          )}
        </MapboxMapComponent>
