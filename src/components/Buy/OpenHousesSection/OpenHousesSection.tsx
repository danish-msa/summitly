...
/** Today in YYYY-MM-DD for Repliers minOpenHouseDate (upcoming open houses) */
...
export default function OpenHousesSection() {
    ...
    const load = async () => {
      try {
        ...
        if (isMounted) {
          ...
          console.error('OpenHousesSection:', err);
        }
      } finally {
  ...
  return (
    <section className="bg-white py-12 md:py-16" aria-labelledby="open-houses-heading">
      <div className="container-1400 mx-auto px-4 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <h2 id="open-houses-heading" className="text-2xl md:text-3xl font-bold text-foreground">
        ...
        {loading ? (
          ...
          <p className="text-muted-foreground py-8">Unable to load open houses. Please try again later.</p>
        ) : listings.length === 0 ? (
          <p className="text-muted-foreground py-8">No upcoming open houses at the moment.</p>
        ) : (
