...
/** Target price for "Homes around $X" */
const TARGET_PRICE = 699_000;
/** Range each side of target (so min = TARGET - RANGE, max = TARGET + RANGE) */
const PRICE_RANGE = 100_000;

export default function BuyPageListingsSection() {
    ...
    const load = async () => {
      try {
        ...
        const result = await RepliersAPI.listings.getFiltered({
          status: 'A',
          type: 'Sale',
          minPrice: TARGET_PRICE - PRICE_RANGE,
          maxPrice: TARGET_PRICE + PRICE_RANGE,
        ...
        if (isMounted) {
          ...
          console.error('BuyPageListingsSection:', err);
        }
      } finally {
  ...
  return (
    <section className="bg-white py-12 md:py-16" aria-labelledby="buy-listings-heading">
      <div className="container-1400 mx-auto px-4 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <h2 id="buy-listings-heading" className="text-2xl md:text-3xl font-bold text-foreground">
            Homes around {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(TARGET_PRICE)}
          </h2>
          <Link
            href="/listings"
