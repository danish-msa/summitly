...
export default function LuxuryHomesSection() {
    ...
    const load = async () => {
      try {
        ...
        if (isMounted) {
          ...
          console.error('LuxuryHomesSection:', err);
        }
      } finally {
  ...
  return (
    <section className="bg-white py-12 md:py-16" aria-labelledby="luxury-homes-heading">
      <div className="container-1400 mx-auto px-4 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <h2 id="luxury-homes-heading" className="text-2xl md:text-3xl font-bold text-foreground">
            Luxury Homes
          </h2>
          <Link
            href="/listings"
        ...
        {loading ? (
          ...
          <p className="text-muted-foreground py-8">Unable to load luxury homes. Please try again later.</p>
        ) : listings.length === 0 ? (
