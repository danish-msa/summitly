...
export default function NewestListingsSection() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!isMounted) return;
        let items = result.listings ?? [];
        // Sort by listDate descending client-side in case API doesn't support sort
        items = [...items].sort((a, b) => {
          const dateA = new Date(a.listDate || 0).getTime();
          const dateB = new Date(b.listDate || 0).getTime();
          return dateB - dateA;
        });
        setListings(items);
      } catch (err) {
        if (isMounted) {
          ...
          console.error('NewestListingsSection:', err);
        }
      } finally {
  ...
  return (
    <section className="bg-white py-12 md:py-16" aria-labelledby="newest-listings-heading">
      <div className="container-1400 mx-auto px-4 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <h2 id="newest-listings-heading" className="text-2xl md:text-3xl font-bold text-foreground">
            Newest Listings
          </h2>
          <Link
            href="/listings"
        ...
        {loading ? (
          ...
          <p className="text-muted-foreground py-8">Unable to load newest listings. Please try again later.</p>
        ) : listings.length === 0 ? (
