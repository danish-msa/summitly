  ...
  return (
    <div className="bg-white p-4 rounded-lg space-y-4">
      ...
      {!showAll && areasData.length > 1 ? (
        ...
        <Tabs value={selectedArea || areasData[0]?.name || ""} onValueChange={setSelectedArea} className="w-full">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 pt-2">
              {selectedAreaData.neighborhoods.map((neighborhood) => (
                <Link
                  ...
                  href={`/listings?location=${slugify(cityName)}&neighborhood=${slugify(neighborhood.name)}`}
                >
                  <Button
                    variant="ghost"
