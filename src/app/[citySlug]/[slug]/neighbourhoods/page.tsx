...
// Helper function to unslugify area name
const unslugifyAreaName = (slug: string): string => {
  return slug
    .split('-')
...
const AreaNeighbourhoodsPage: React.FC = () => {
  const params = useParams();
  const citySlug = params?.citySlug as string || '';
  const areaSlug = params?.slug as string || ''; // Changed from areaName to slug
  const [loading, setLoading] = useState(true);
  const [areaInfo, setAreaInfo] = useState<{ name: string; cityName: string } | null>(null);
  ...
  // Parse area name from slug
  const areaName = useMemo(() => {
    return unslugifyAreaName(areaSlug);
  }, [areaSlug]);
  ...
  useEffect(() => {
    const loadAreaData = async () => {
      try {
        ...
        // Filter properties by city and area
        const areaProperties = listingsData.listings.filter(property =>
          property.address?.city?.toLowerCase() === cityName.toLowerCase() &&
          property.address?.area?.toLowerCase() === areaName.toLowerCase()
        );
    ...
    if (citySlug && cityName && areaSlug && areaName) {
      loadAreaData();
    }
  }, [citySlug, cityName, areaSlug, areaName]);
  ...
  const displayAreaName = areaInfo?.name || areaName;
  const displayCityName = areaInfo?.cityName || cityName;
          ...
          <TableBody className="bg-white">
            {data.length > 0 ? (
              data.map((neighbourhood) => {
                const neighbourhoodUrl = `/${citySlug}/${areaSlug}/${slugifyNeighbourhoodName(neighbourhood.name)}`;
    ...
    <div className="min-h-screen">
          ...
          <div className="mb-4">
            <Link href={`/${citySlug}/${areaSlug}`}>
              ...
              >
                ...
                <span>Back to {displayAreaName} Page</span>
              </Button>
            </Link>
          </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <h1 className="text-3xl font-bold text-foreground mb-2">
                The Hottest {displayAreaName} Neighbourhoods
              </h1>
              <p className="text-muted-foreground mt-4">
                Get an idea of the fastest selling neighbourhoods in {displayAreaName}, {displayCityName}. See areas with the most demand, as measured by competitive bids over asking price. And, evaluate the average selling price of each neighbourhood.
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="container-1400 mx-auto px-4 py-8 space-y-4">
        {neighbourhoodData ? (
          <>
            <NeighbourhoodTable 
              title={`Hottest ${displayAreaName} Neighbourhoods`}
            ...
            <NeighbourhoodTable 
              title={`Coldest ${displayAreaName} Neighbourhoods`}
            ...
            <NeighbourhoodTable 
              title={`All ${displayAreaName} Neighbourhoods`}
          ...
          <div className="bg-secondary/30 rounded-lg p-12 text-center">
            <p className="text-lg text-muted-foreground">
              No neighbourhood data available for {displayAreaName}
            </p>
          </div>
        )}
      </main>
...
export default AreaNeighbourhoodsPage;
