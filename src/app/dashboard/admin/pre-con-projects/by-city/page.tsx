...
// Get city names from the data (using the id as pageValue)
const CITIES = preConCityProjectsData.map(city => ({
  id: city.id,
  name: city.name,
}))

export default function ByCityPage() {
  ...
  const fetchPageContents = async () => {
    await fetchData(async () => {
      const response = await fetch("/api/admin/pre-con-projects/page-content?pageType=city")
      if (!response.ok) {
      ...
      // Create entries for all cities, merging with existing content
      const allContents: PageContent[] = CITIES.map((city) => {
        const existing = contents.find((c: PageContent) => c.pageValue === city.id)
        return existing || {
          id: `new-${city.id}`,
          pageType: "city",
          pageValue: city.id,
  ...
  const filteredContents = pageContents.filter((content) => {
    const city = CITIES.find(c => c.id === content.pageValue)
    const cityName = city?.name || content.pageValue
    return (
  ...
  const columns: Column<PageContent>[] = [
    {
      ...
      render: (content) => {
        const city = CITIES.find(c => c.id === content.pageValue)
        return (
          <div className="font-medium">{city?.name || content.pageValue}</div>
        )
      },
    },
    {
      ...
      render: (content) => (
        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => router.push(`/dashboard/admin/pre-con-projects/by-city/${encodeURIComponent(content.pageValue)}/edit`)}
          >
