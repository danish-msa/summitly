...
export default function EditSellingStatusPage() {
  ...
  const fetchPageContent = async (pageValue: string) => {
    try {
      const content = await fetchData(async () => {
        const response = await fetch(`/api/admin/pre-con-projects/page-content?pageType=status&pageValue=${encodeURIComponent(pageValue)}`)
        if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push("/dashboard/admin/pre-con-projects/by-selling-status")
    } catch (error) {
  ...
  return (
    <div className="space-y-6 pb-32">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit Selling Status Page</h1>
          <p className="text-muted-foreground mt-1">
            {formatStatusLabel(pageValue)} - Customize page content
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Customize the content for this selling status page. If left empty, the default filtered view will be shown.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="e.g., Now Selling - Exclusive Pre-Construction Projects"
              />
            </div>
