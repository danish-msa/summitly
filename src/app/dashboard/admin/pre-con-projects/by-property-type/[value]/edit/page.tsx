...
export default function EditPropertyTypePage() {
  ...
  const fetchPageContent = async (pageValue: string) => {
    try {
      const content = await fetchData(async () => {
        const response = await fetch(`/api/admin/pre-con-projects/page-content?pageType=propertyType&pageValue=${encodeURIComponent(pageValue)}`)
        if (!response.ok) {
          if (response.status === 404) {
            // Page content doesn't exist yet, that's okay
            return null
          }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push("/dashboard/admin/pre-con-projects/by-property-type")
    } catch (error) {
  ...
  return (
    <div className="space-y-6 pb-32">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit Property Type Page</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Customize the content for this property type page. If left empty, the default filtered view will be shown.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="e.g., Luxury Condos in Toronto"
              />
            </div>
