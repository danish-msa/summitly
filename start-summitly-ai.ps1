...
if (-not (Test-Path $envPath)) {
    if (Test-Path $envExamplePath) {
        ...
        Write-Host "⚠️  No .env.local found. Please create one with required environment variables." -ForegroundColor Yellow
    }
}
...
if (Test-Path "venv\Scripts\Activate.ps1") {
    ...
    Write-Host "⚠️  Could not find venv activation script" -ForegroundColor Yellow
}
...
Write-Host "`n✅ Summitly AI services are starting!" -ForegroundColor Green
Write-Host "=" * 70 -ForegroundColor Cyan
...
Write-Host "=" * 70 -ForegroundColor Cyan
Write-Host "`n💡 Tip: Both services are running in separate windows. Close those windows to stop them." -ForegroundColor Yellow
